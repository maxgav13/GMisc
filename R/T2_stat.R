...
#' @export
#' @return A ggplot object showing the Mahalanobis D2 statistic and lines marking the critical values at 0.95, 0.99, and 0.999
...
T2_stat = function(x, k = 51) {
  ...
  q = ggplot(Data, aes_string(nombres[1], "D2")) +
    geom_line(na.rm = T) +
    geom_hline(yintercept = chi, col = c("blue", "orange", "red")) +
    coord_flip() + scale_x_reverse(name = "Depth [m]") +
    scale_y_continuous(name = "Mahalanobis D2") +
    theme_bw()
