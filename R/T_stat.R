...
#' @export
#' @return A ggplot object showing the Cohen's d statistic and lines marking the Cohen's U3 values for 0.95, 0.99, and 0.999
...
T_stat = function(x, k = 7) {
  ...
  q = ggplot(Data, aes_string(nombres[1], "d")) +
    geom_line(na.rm = T) +
    geom_hline(yintercept = d_crit, col = c("blue", "orange", "red")) +
