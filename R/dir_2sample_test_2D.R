...
dir_2sample_test_2D = function(x, y, dir = 1, conf.level = 0.95) {
  ...
  r1 = dir_stats_2D(x, dir = dir, conf.level = conf.level)
  r2 = dir_stats_2D(y, dir = dir, conf.level = conf.level)
  r3 = dir_stats_2D(xy, dir = dir, conf.level = conf.level)
  ...
  f = ifelse(kt > 10, ((length(xy) - 2) * (R1 + R2 - Rt)) / (length(xy) - R1 - R2),
             (1 + 3 / (8 * kt)) * ((length(xy) - 2) * (R1 + R2 - Rt)) / (length(xy) - R1 - R2))
  fcrit = qf(conf.level, 1, length(xy) - 2)
  p = pf(f, 1, length(xy) - 2, lower.tail = F)
  ...
  interpretation = ifelse(f > fcrit,
                          ...
                          paste("Do not reject H0 and conclude that there is not enough evidence that the two samples could not come from the same population with the same mean direction"))
