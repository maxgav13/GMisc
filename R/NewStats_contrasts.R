...
#'
NewStats_contrasts = function(dat, g1, g2, col.g1 = 3, col.g2 = 4, col.diff = 2, ylab = 'Values', ylab.diff = 'Difference', B.labels = c('G1', 'G2', 'Difference')) {
  ...
  for (i in 1:nrow(dat)) {
    dat$upper[i]=ci_t(dat[i,1],dat[i,2],dat[i,3])$upper
  }

  for (i in 1:nrow(dat)) {
    dat$lower[i]=ci_t(dat[i,1],dat[i,2],dat[i,3])$lower
  }
  ...
  lower = c(ci_t(mean(G1$mean),mean(G1$sd),sum(G1$n))$lower,
            ci_t(mean(G2$mean),mean(G2$sd),sum(G2$n))$lower,
            ci_t2(mean(G1$mean),mean(G1$sd),sum(G1$n),mean(G2$mean),mean(G2$sd),sum(G2$n))$lower)

  upper = c(ci_t(mean(G1$mean),mean(G1$sd),sum(G1$n))$upper,
            ci_t(mean(G2$mean),mean(G2$sd),sum(G2$n))$upper,
            ci_t2(mean(G1$mean),mean(G1$sd),sum(G1$n),mean(G2$mean),mean(G2$sd),sum(G2$n))$upper)
